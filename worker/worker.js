...
async function accessToken(env) {
  ...
  if (!r.ok) throw fail("Google auth failed: " + (j.error_description || j.error), 502);
...
async function handleBook(body, env) {
  const { start, end, mode, name, email, notes, viewerTz } = body || {};
  ...
  const event = {
    summary: `HOLD — ${mode === "virtual" ? "Virtual" : "In person"} · ${clean(name, 60)}`,
    description:
